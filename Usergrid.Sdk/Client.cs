...
namespace Usergrid.Sdk
{
    public class Client : IClient
    {
        ...
        public UsergridEntity<T> CreateEntity<T>(string collection, T entity) where T : class
        {
        ...
        public UsergridEntity<T> GetEntity<T>(string collectionName, string identifer /*username or uuid or email*/)
        {
        ...
        {
            UsergridEntity<T> user = GetEntity<T>("users", identifer);
            if (user == null)
                return null;
        ...
        {
            UsergridEntity<T> usergridEntity = EntityManager.GetEntity<T>("groups", identifer);
        ...
        {
            EntityManager.CreateEntity<object>(string.Format("/groups/{0}/users/{1}", groupIdentifier, userName));
        }
        ...
        {
            return ConnectionManager.GetConnections(connector, connection);
        }
        ...
        {
            UsergridEntity<T> device = GetEntity<T>("devices", identifer);
            if (device == null)
                return null;
        ...
        {
            UsergridEntity<T> usergridNotifier = EntityManager.GetEntity<T>("/notifiers", identifer);
